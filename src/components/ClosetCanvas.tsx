...
export const ClosetCanvas = ({
  ...
  const getCanvasStyle = () => {
    if (isMobile) {
      return {
        width: '100%',
        height: Math.min(400, window.innerHeight * 0.4), // Smaller height for mobile
  ...
  return (
    <div className="bg-white rounded-2xl p-2 sm:p-4 shadow-lg border border-gray-200">
      ...
      <div
        ref={canvasRef}
        className="relative border-2 border-gray-300 rounded-lg overflow-hidden w-full"
      ...
      >
        ...
        >
          {(() => {
            const canvasHeight = isMobile ? Math.min(400, window.innerHeight * 0.4) : CANVAS_CONFIG.height;
            const canvasWidth = isMobile ? window.innerWidth - 32 : CANVAS_CONFIG.width; // Account for padding
            ...
            return (
              <>
                ...
                <line 
                  ...
                  strokeWidth="2"
                />
                {/* Right wall - responsive */}
                <line 
                  x1={isMobile ? "calc(100% - 20px)" : CANVAS_CONFIG.width - 20} 
                  y1="0" 
                  x2={isMobile ? "calc(100% - 20px)" : CANVAS_CONFIG.width - 20} 
                  y2={canvasHeight - 20}
                  stroke="rgba(0,0,0,0.3)" 
                  strokeWidth="2"
                />
